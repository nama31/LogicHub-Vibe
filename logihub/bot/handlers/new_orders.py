...
async def new_orders_handler(message: Message, courier_tg_id: int, order_service: BotOrderService) -> None:
	"""Показать новые (назначенные) заказы курьера."""

	orders = await order_service.get_courier_orders(tg_id=courier_tg_id, status_filter="assigned")
